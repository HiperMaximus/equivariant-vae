...
from typing import TYPE_CHECKING, Final, Protocol, Self, TypedDict, cast
...
if TYPE_CHECKING:
    ...
    class WSIInstance(Protocol):
        """Structural coordinate view required by graph construction."""
...
class LocalGlobalMILClassifier(nn.Module):
    ...
    def forward_with_representations(
        ...
        """Return logits and the six fixed T0 semantic representation terminals.

        This state-compatible entry point exists only for the instrumented
        Spec 0054 wrapper. It does not alter the ordinary historical forward.
        """
        x0 = cast("Tensor", self.patch_encoder(latents))
        _validate_tokens_and_graph(x0, graph)
        x1 = cast("Tensor", self.local_blocks[0](x0, graph))
        x2 = cast("Tensor", self.local_blocks[1](x1, graph))
        t1 = cast("Tensor", self.global_summary(self.global_tokens, x2))
        cls = cast("Tensor", self.cls_block(t1))
        with torch.autocast(device_type=cls.device.type, enabled=False):
            bag_embedding = cast("Tensor", self.final_norm(cls.float()))
            logits = functional.linear(
                bag_embedding,
                self.classifier.weight.float(),
                self.classifier.bias.float(),
            )
        return logits, (x0, x1, x2, t1, cls, bag_embedding)
