...
SCHEMA_VERSION: Final = "spec0013.so2_dual_t4_probe.v1"
ARTIFACT_FILENAME: Final = "spec0013_so2_dual_t4_probe.json"
PROBE_KIND: Final = "locked_so2_architecture_mechanics"
...
def _time_block(
    ...
    *,
    ...
    def step() -> tuple[torch.Tensor, bool, int]:
        optimizer.zero_grad(set_to_none=True)
        ...
        nonfinite_gradients = sum(
            int(not bool(torch.isfinite(parameter.grad).all().item()))
        ...
        scaler.step(optimizer)
        scaler.update()
        return loss.detach(), scaler.get_scale() < previous_scale, nonfinite_gradients
...
def _coefficient_gradient_comparison(
    ...
    for name in sorted(set(eager_coefficients) & set(compiled_coefficients)):
        ...
        errors.append(_relative_rms(compiled_gradient, eager_gradient))
    return max(errors, default=0.0), sorted(set(missing)), nonfinite


def _accuracy_case(case: _BlockCase, device: torch.device) -> dict[str, object]:
    _manual_seed(330013)
    ...
    compiled_loss.backward()  # pyright: ignore[reportUnknownMemberType]
    gradient_error, missing_gradients, nonfinite_gradients = (
...
def run(output_dir: Path) -> JsonObject:
    ...
    rows, local_weighted_ratio = _measure_blocks(distributed)
    local_assembly_fraction = _assembly_fraction(trained_suite, distributed)
    rank_results, weighted_ratio, assembly_fraction = _gather_rank_measurements(
        rows,
        local_weighted_ratio,
        local_assembly_fraction,
        distributed,
    )
    ...
    result = cast(
        "JsonObject",
        {
            ...
            "follow_up_probe_permitted": not passed,
