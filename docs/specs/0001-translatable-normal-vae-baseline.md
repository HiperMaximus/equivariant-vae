...
markers below are load-bearing literals grepped by `scripts/kaggle_kernel.sh`
build/push guards and `docs/specs/README.md` — do not rename them, and do not
change this readiness line to `ready` / `implementation-ready` (that flips the
fail-closed broad-implementation push guard to PASS):

- capped real-data Kaggle smoke: `kaggle_smoke_ready`
- no-dataset synthetic binary Kaggle timing pretest contract:
  `kaggle_synthetic_timing_contract_ready`
- capped real-data runtime pretest config/schema contract:
  `real_data_runtime_pretest_contract_ready`
- completed selected-runtime benchmark shortlist slice:
  `v8_shortlist_eager_amp_then_dual_gate`
- selected-runtime debug/resume/tiny Kaggle gate contract:
...
8. `benchmark/selected_runtime.json` may be written with `status = "pass"` only
   ...
   This promotion decision is row-scoped by design: a v4 benchmark false-negative
   that fail-closed the whole selection on small bounded numerical drift and on
   non-selected rows' proof failures was fixed in `fc5227d`.
