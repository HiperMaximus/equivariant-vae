...
mutation-proof buffer tests. Then `2269050` (fp32 buffer, filled in place), `72ef19e` **A1**
...
- **S17** Accept the compiled plan + activate the deferred observation, then run the
  ...
  - **S17-Kaggle** [Kaggle] Run the S14 generator on dual-T4 → new compiled
    ...
    NOT pinned to `amp_off_fp32` — A2 exists so fp16 can win — and corruption moved off blake2b
    in `5dde097`, so `indexed_masked` is no longer the expected term either). With the
