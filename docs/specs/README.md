...
| `0003-kaggle-cli-execution-workflow.md` | draft active workflow scaffold; synthetic setup-smoke remote v1 passed; synthetic timing remote v4 passed; real-data runtime pretest remote v4/v5/v6/v7/v8 artifacts are downloaded; selected-runtime kernel path is guarded as `runtime_selection_kernel_ready` | Runtime-selection v3 downloaded to `runs/kaggle/runtime_selection_v3`, proved real dual-T4 DDP timing, wrote `benchmark/selected_runtime.json`, and confirmed the selected row. Runtime-selection v4 downloaded to `runs/kaggle/runtime_selection_v4` and failed closed on proof-policy false negatives; local replay after `fc5227d` passes. Runtime-selection v5 is running the corrected efficiency follow-up. Full training/full-run launchers are still not implemented. | User approved the selected-runtime efficiency-follow-up Kaggle push/status/output with the normal guard variables. Other remote reads/writes still need explicit approval plus `KAGGLE_REMOTE_CONFIRMED=1` for reads/status/output and `KAGGLE_PUSH_CONFIRMED=1 KAGGLE_FULL_DATASET_CONFIRMED=1` for pushes with real data attachments. For long-running Kaggle jobs, do one immediate status check, then give the user a concrete `continue` prompt time instead of waiting in-turn; next prompt is 2026-06-21 11:15 -05 for runtime-selection v5. |
...
`pass` for the intended AMP row. Runtime-selection v5 is running the corrected
follow-up; it can replace v3 only if the remote proof artifact passes and no
catastrophic failure appears.
