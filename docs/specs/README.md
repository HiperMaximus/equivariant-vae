...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 5's terminal archive verifies continuation through update 45000. The session-4 67/68 caveat remains, and session 5 adds an accepted 66/68 gate result. | Locally ready session-6 transport awaits exact remote authorization. | Use only hash-pinned `step_045000.pt`; do not upload or launch before approval. |
