...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 5's terminal archive verifies continuation through update 45000. The session-4 67/68 caveat remains, and session 5 adds an accepted 66/68 gate result; session-6 v3 and unchanged v4 both failed before checkpoint load because Kaggle omitted the 45k input, despite the successful one-source mount probe. | Inspect accepted full-kernel metadata and run an exact two-source mount-only probe before any retry. | Do not infer the next checkpoint before its proof and manifest verify. |
