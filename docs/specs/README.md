...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; sessions 1-4 are verified through update 36000, with continuation accepted under the recorded 67/68 gate caveat. The namespaced mount correction, post-verification CUDA/PyPI evidence, focused preflight, and full quality gate pass locally. | Seek new exact authorization for the session-5 retry. | Use only the hash-pinned `step_036000.pt` lineage; never infer the next checkpoint from an old wrapper. |
