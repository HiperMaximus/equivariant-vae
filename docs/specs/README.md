...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 6 verifies update 54000/all 13 hashes. Its 66/68 result with one and five saturated-open channels is accepted; final kernel v2 failed before load, and fresh 54k mount-probe v1 remains `RUNNING` after a bounded 20-minute wait with empty logs. | With fresh read authorization, check that same probe version only after a reasonable interval; retry training only after an exact mount pass. | Preserve update 54000 as sole authority, do not weaken the gate caveat, and do not resubmit the probe. |
