...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 5's terminal archive verifies continuation through update 45000. The session-4 67/68 caveat remains, and session 5 adds an accepted 66/68 gate result; session-6 v3 and unchanged v4 both failed before checkpoint load despite verified one- and two-source mounts. The exact Torch-upgrade preamble probe is running before code output. | Read the preamble probe once it settles; do not retry training yet. | Do not infer the next checkpoint before its proof and manifest verify. |
