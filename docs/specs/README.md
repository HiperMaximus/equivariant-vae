...
| `0011-reusable-goal-derived-runtime-and-compiled-fastpath.md` | draft active; **Phase 1 (S1–S10) + Phase 2 (S11–S13) committed through `8e14650`; Phase 3 S15 DONE (local)** (Spec 0011 S15); S16 local next; Kaggle phases S14/S17/S19 gated | Promote the compiled fast-path + probe-derived bigger batch into the first paper-promotable full run, built as a REUSABLE per-(model×hardware) SEARCH-then-run mechanism (the search emits `selected_runtime.json`; config carries no batch/LR; validators check relationships, not literals) so the future equivariant model re-runs it unchanged. Supersedes Spec 0009's frozen schedule. Design = 17 steps (4 phases) + adversarial critique (5 must-fixes) folded in; drop_last=True kept → `floor(P/G)` schedule; provenance = folded honest generator. See the per-step `(DONE — …)` tags in the spec body for the state of record. | **NEXT = S16** (local, Phase 3): compiled whole-step (`torch.compile(step, dynamic=False)`) + `drop_last=True` flip + wire the S15-deferred dynamo config. (**S15 DONE**, local: the runner consumes the DDP recipe + fused optimizer via `src/eqvae/training/fastpath_recipe.py`, plan-gated / behavior-preserving-off, with a **structural** `broadcast_buffers` rule; the plan-applied observation mirror is deferred.) Kaggle-only (user-driven): S14 (fold the probe sweep + winner-recipe DDP timing into `runtime_selection_executor` and actually run a `step` row — extend the executor guards), S17 (run generator → new `selected_runtime.json`), S19 (~30h full run). Each step = one gated commit + adversarial review + explicit approval, in a NEW window per step; never push to origin. |
