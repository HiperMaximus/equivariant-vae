...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; sessions 1-4 are verified through update 36000, with continuation accepted under the recorded 67/68 gate caveat. Probes proved the namespaced checkpoint mount; post-verification CUDA and PyPI Torch-upgrade dry-run evidence pass. | Correct and preflight only the mount path, then seek new exact authorization. | Use only the hash-pinned `step_036000.pt` lineage; never infer the next checkpoint from an old wrapper. |
