...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; sessions 1-4 are verified through update 36000, with continuation accepted under the recorded 67/68 gate caveat. | A legitimate Kaggle operator must independently authenticate, verify public-UBC attachment, and receive an exactly authorized update-36000 checkpoint transport. | Repin and verify session 5 from only the hash-pinned `step_036000.pt`; never push the still-update-27000 launcher unchanged. |
