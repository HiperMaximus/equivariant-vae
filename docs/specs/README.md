...
| `0002-strict-python-quality-gate.md` | active strict gate; semantic test-intent and redundancy contract added 2026-08-02; the last broad pass predates the Spec 0011 cleanup | The current dirty tree needs a fresh complete gate after winner-plan/LR-finder work. | Keep only cheap unique goal-relevant tests and rerun the repo quality gate before commit/push. |
| `0003-kaggle-cli-execution-workflow.md` | draft active workflow scaffold; the CLI push/status/output path is proven; historical v5 is not the new selected runtime; Spec 0009 adds dedicated full-run actions; full-kernel v1 was canceled | Canceled v1 has resumable checkpoints but no metrics/benchmark, so strict verification fails. Remote reads/writes require explicit approval plus confirmation variables. | Maintain the guarded CLI while the Spec 0011 winner is translated and checked by the short real dual-T4 gate. |
...
| `0008-canonical-fixed32-and-remote-debug-tiny-readiness.md` | implemented; historical v5 debug/tiny output passes strict verification | None within Spec 0008; its proof remains a prerequisite pattern, not approval of the new runtime. | Run the same short real dual-T4 gate against the translated Spec 0011 winning plan. |
| `0009-first-full-selected-runtime-training-run.md` | Historical v1 canceled after `step_043750.pt`; its durability/DDP lessons remain, but its batch-24/125000-update runtime literals are superseded by Spec 0011. | V1 is not a resume base. The current global-50/60000-update run uses separate checkpoint-only sessions and retains their outputs locally for a final merge. | Verify the narrow Spec 0011 session path, then request separate session-1 approval. |
...
| `0011-reusable-goal-derived-runtime-and-compiled-fastpath.md` | v5 lean contract; runtime/LR/debug/learnability and paired-beta probes passed on real dual-T4; beta `0.01` locked; checkpoint-only session continuation selected. | Remove completed beta-probe wiring; verify boundary checkpoint/evaluation durability, index-only resume, per-rank RNG rebasing, beta-`0.01` full kernel, broad gates, and clean launch review. | Finish local session-1 preparation, then request separate explicit approval for its guarded push. |
