...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; sessions 1-4 are verified through update 36000, with continuation accepted under the recorded 67/68 gate caveat; session 5 version 1 is running. | Wait for terminal status, then verify its separately downloaded proof before choosing another resume point. | Use only the hash-pinned `step_036000.pt` lineage; never infer the next checkpoint from an old wrapper. |
