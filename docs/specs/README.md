...
| `0008-canonical-fixed32-and-remote-debug-tiny-readiness.md` | local readiness implemented / locally verified; remote proof pending | v3 was approved, pushed, downloaded to `runs/kaggle/selected_runtime_debug_v3`, and is not a passing remote proof. It proved canonical selector generation plus debug/resume/plan/gate/manifest artifact writing, but tiny-overfit failed on AMP/nonfinite rows caused by fixed-32 DDP tail microbatches. Local follow-up keeps the partial-batch eps source fix and adds a tiny-only fixed32 full-batch repeated sampler plus verifier/wrapper evidence checks for sampler policy, effective epoch samples, observed bs12 tiny batches, zero tiny AMP skips, and zero aggregate nonfinite rows. After explicit approval, v4 was pushed from clean commit `ce72fa0` and the immediate guarded status read at `2026-06-29 01:11:09 -0500` returned `KernelWorkerStatus.RUNNING`. | Next action is a guarded v4 status poll after about `2026-06-29 01:45 -0500`; if terminal, download to `runs/kaggle/selected_runtime_debug_v4` and run `eqvae.cli.selected_runtime_gate --verify-output`. This is not approval for the first full long run. |
