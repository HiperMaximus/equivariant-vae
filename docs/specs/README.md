...
| `0009-first-full-selected-runtime-training-run.md` | version 1 canceled after checkpoint `step_043750.pt`; resume-policy patch needed | Kaggle accepted `maximusshtefan/eqvae-selected-runtime-full` version 1, then returned `KernelWorkerStatus.CANCEL_ACKNOWLEDGED`. Downloaded outputs include resumable checkpoints but no `metrics/` or `benchmark/`; current full-run resume requires prior train metric history, so a naive resume push would fail. | Patch and test canceled-output checkpoint resume behavior locally, including interval metric flushing for future runs and explicit verifier policy for the v1 checkpoint-only prefix. |
