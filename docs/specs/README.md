...
| `0008-canonical-fixed32-and-remote-debug-tiny-readiness.md` | local readiness implemented / locally verified; remote proof pending | v2 proved the canonical real fixed-32 selector on Kaggle but failed before training artifacts on a partial-batch explicit-epsilon shape bug. Local follow-up fixes the source runner, switches the wrapper to torch-distributed two-rank launches, and hardens downloaded-output verification. v3 was approved, pushed, and is currently running. | Next action is to poll v3 after the recorded cadence, download outputs when terminal, then run `eqvae.cli.selected_runtime_gate --verify-output`. If remote artifacts pass, the first full real selected-runtime run becomes the next candidate action. |
