...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 5 verifies update 45000 with retained 67/68 and accepted 66/68 caveats. Session-6 v3-v5 fail before checkpoint load on existing resource `131085532`; all fresh controls pass, and a fresh full-kernel ID passes local gates. | Obtain exact authorization and launch the unchanged continuation as `maximshtefan/eqvae-so2-selected-runtime-full-session6`. | Do not infer the next checkpoint before its proof and manifest verify. |
