...
| `0008-canonical-fixed32-and-remote-debug-tiny-readiness.md` | local readiness implemented / locally verified; remote proof pending explicit approval | v2 proved the canonical real fixed-32 selector on Kaggle but failed before training artifacts on a partial-batch explicit-epsilon shape bug. Local follow-up fixes the source runner, switches the wrapper to torch-distributed two-rank launches, and hardens downloaded-output verification; a new remote proof is still required. | Next action is to request explicit approval for the narrow selected-runtime debug/tiny v3 Kaggle push when the user is ready; after download, run `eqvae.cli.selected_runtime_gate --verify-output`. If remote artifacts pass, the first full real selected-runtime run becomes the next candidate action. |
