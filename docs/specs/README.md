...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 5's terminal archive verifies continuation through update 45000. The session-4 67/68 caveat remains, and session 5 adds a 66/68 gate result. | User decision on whether to continue under the 66/68 result, then new exact authorization. | Use only hash-pinned `step_045000.pt`; do not create transport or launch before both approvals. |
