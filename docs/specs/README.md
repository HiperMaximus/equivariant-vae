...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; sessions 1-4 are verified through update 36000, with continuation accepted under the recorded 67/68 gate caveat. The namespaced mount correction, post-verification readiness evidence, focused preflight, and full quality gate passed; session-5 version 2 is running. | Download and validate its terminal artifact only; do not infer a boundary. | Use only the hash-pinned `step_036000.pt` lineage; never infer the next checkpoint from an old wrapper. |
