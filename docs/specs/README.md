...
| `0008-canonical-fixed32-and-remote-debug-tiny-readiness.md` | local readiness implemented / locally verified; remote proof pending explicit approval | Requires exact real-dataset metadata attachment and explicit user approval before any Kaggle remote action. The canonical real selector is generated and validated inside the approved remote debug/tiny kernel before training. | Next action is to request explicit approval for the narrow selected-runtime debug/tiny Kaggle push when the user is ready; after download, run `eqvae.cli.selected_runtime_gate --verify-output`. If remote artifacts pass, the first full real selected-runtime run becomes the next candidate action. |
