...
| `0003-kaggle-cli-execution-workflow.md` | draft active workflow scaffold; synthetic setup-smoke push path ready after permission | The setup-smoke kernel now launches a generated embedded no-dataset setup path. The real-data capped smoke still needs embedded packaging before rerun. Full benchmark/full-run launchers are still not implemented. | Use `./scripts/kaggle_kernel.sh build kaggle/kernels/setup_smoke` before any approved setup-smoke push. Remote writes still require explicit user permission and `KAGGLE_PUSH_CONFIRMED=1`. |
