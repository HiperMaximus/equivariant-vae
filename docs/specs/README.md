...
| `0013-fixed-f01-architecture-probe.md` | Fixed local mechanics implemented and verified: 64 focused tests pass, exact counts reproduce, and the guarded dual-T4 runner is locally ready. | Clean reviewed source commit, fresh explicit permission for the first Kaggle write, and passing dual-T4 evidence. | Commit this slice, then run only the selected baseline bundle on dual T4 after permission; stop if every limit passes. |
