...
| `0016-so2-real-data-prelaunch-and-full-run.md` | Locked; session 5's terminal archive verifies continuation through update 45000. The session-4 67/68 caveat remains, and session 5 adds an accepted 66/68 gate result; session-6 v3 failed before checkpoint load during delayed Kaggle attachment provisioning, later confirmed by a correct mount-only probe. Unchanged v4 is submitted through update 60000. | Wait for v4 status and download its proof if terminal. | Do not infer the next checkpoint before its proof and manifest verify. |
